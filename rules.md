...
- Non-trivial logic (the notify route's validation + send path; likewise Phase 4's review flow) leaves **one** runnable check behind — the smallest thing that fails if it breaks. No test framework, no fixtures, no suites.
...
- **No storage layer.** The admin inbox is the record (see architecture.md §5). Do not add a database, KV, or spreadsheet integration — except the review store (Phase 4, owner-added): Upstash Redis holds reviews only; signups still land in the inbox.
- **Recipient email is a hardcoded constant** (`stsales@shizentsunagi.com`). Env vars: `RESEND_API_KEY`, the Redis pair, `MODERATION_SECRET` — nothing else. The Redis pair is read under **either** the spec names (`UPSTASH_REDIS_REST_URL` / `UPSTASH_REDIS_REST_TOKEN`) **or** Vercel's Upstash-integration names (`KV_REST_API_URL` / `KV_REST_API_TOKEN` — the write token, never `KV_REST_API_READ_ONLY_TOKEN`). The platform already injects the latter; hand-duplicating them into alias names is config for a value that already exists (ladder rung 1).
