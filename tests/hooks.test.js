...
const { DEFAULT_MODE, getDefaultMode, isShellSafe, writeDefaultMode } = require('../hooks/shizen-config');
assert.equal(isShellSafe('C:\\Users\\x\\.claude\\plugins\\shizen\\hooks\\shizen-statusline.ps1'), true);
assert.equal(isShellSafe('/home/u/.claude/plugins/shizen/hooks/shizen-statusline.sh'), true);
...
delete process.env.SHIZEN_SUBAGENT_MATCHER;
delete process.env.QODER_SESSION_ID;

const temp = fs.mkdtempSync(path.join(os.tmpdir(), 'shizen-hooks-'));
...
const codexEnv = {
  ...
  SHIZEN_DEFAULT_MODE: 'ultra',
};
const codexState = path.join(pluginData, '.shizen-active');

let result = run('shizen-activate.js', codexEnv);
assert.equal(result.status, 0, result.stderr);
assert.equal(fs.readFileSync(codexState, 'utf8'), 'ultra');
let output = JSON.parse(result.stdout);
assert.equal(output.systemMessage, 'SHIZEN:ULTRA');
...
assert.match(
  ...
  /SHIZEN MODE ACTIVE — level: ultra/,
);

result = run(
  'shizen-mode-tracker.js',
  codexEnv,
  JSON.stringify({ prompt: '@shizen lite' }),
);
assert.equal(result.status, 0, result.stderr);
assert.equal(fs.readFileSync(codexState, 'utf8'), 'lite');
output = JSON.parse(result.stdout);
assert.equal(output.systemMessage, 'SHIZEN:LITE');

// Querying bare @shizen should report the active level ('lite') without resetting it to default ('ultra')
result = run(
  'shizen-mode-tracker.js',
  codexEnv,
  JSON.stringify({ prompt: '@shizen' }),
);
assert.equal(result.status, 0, result.stderr);
assert.equal(fs.readFileSync(codexState, 'utf8'), 'lite');
output = JSON.parse(result.stdout);
...
output = JSON.parse(result.stdout);
assert.equal(output.systemMessage, 'SHIZEN:OFF');

// A request that merely mentions "normal mode" must not deactivate shizen.
result = run('shizen-mode-tracker.js', codexEnv, JSON.stringify({ prompt: '@shizen lite' }));
assert.equal(result.status, 0, result.stderr);
assert.equal(fs.readFileSync(codexState, 'utf8'), 'lite');
...
assert.equal(
  ...
  'lite',
  'incidental "normal mode" in a request must not turn shizen off',
);

const claudeEnv = {
  HOME: home,
  USERPROFILE: home,
  SHIZEN_DEFAULT_MODE: 'full',
};
delete claudeEnv.PLUGIN_DATA;

result = run('shizen-activate.js', claudeEnv);
assert.equal(result.status, 0, result.stderr);
assert.equal(
  fs.readFileSync(path.join(home, '.claude', '.shizen-active'), 'utf8'),
  'full',
);
...
result = run('shizen-activate.js', {
  HOME: home2,
  USERPROFILE: home2,
  CLAUDE_CONFIG_DIR: customConfigDir,
  SHIZEN_DEFAULT_MODE: 'lite',
});
assert.equal(result.status, 0, result.stderr);
assert.equal(
  fs.readFileSync(path.join(customConfigDir, '.shizen-active'), 'utf8'),
  'lite',
);
assert.equal(
  fs.existsSync(path.join(home2, '.claude', '.shizen-active')),
  false,
...
assert.ok(
  fs.existsSync(path.join(customConfigDir, '.shizen-statusline-nudged')),
  'first nudge must write the once-only flag (#483)',
);
const secondNudge = run('shizen-activate.js', {
  HOME: home2,
  USERPROFILE: home2,
  CLAUDE_CONFIG_DIR: customConfigDir,
  SHIZEN_DEFAULT_MODE: 'lite',
...
assert.equal(result.status, 0, result.stderr);
assert.equal(fs.readFileSync(path.join(copilotData, '.shizen-active'), 'utf8'), 'full');
assert.equal(
  fs.existsSync(path.join(codexData, '.shizen-active')),
  false,
  'copilot hooks must not write mode state to codex PLUGIN_DATA',
);
output = JSON.parse(result.stdout);
assert.match(output.additionalContext, /SHIZEN MODE ACTIVE — level: full/);
...
result = run(
  ...
  JSON.stringify({ prompt: '/shizen ultra' }),
);
assert.equal(result.status, 0, result.stderr);
assert.equal(fs.readFileSync(path.join(copilotData, '.shizen-active'), 'utf8'), 'ultra');
assert.equal(
  fs.existsSync(path.join(codexData, '.shizen-active')),
  false,
...
// SubagentStart hook: when shizen mode is active it injects the ruleset into
...
const subFlag = path.join(subHome, '.claude', '.shizen-active');
...
result = run('shizen-subagent.js', subEnv);
assert.equal(result.status, 0, result.stderr);
output = JSON.parse(result.stdout);
assert.equal(output.hookSpecificOutput.hookEventName, 'SubagentStart');
assert.match(
  output.hookSpecificOutput.additionalContext,
  /SHIZEN MODE ACTIVE — level: full/,
);

// No flag → shizen off → inject nothing (empty stdout, no failure).
fs.unlinkSync(subFlag);
result = run('shizen-subagent.js', subEnv);
assert.equal(result.status, 0, result.stderr);
assert.equal(result.stdout, '', 'SubagentStart must stay silent when shizen is off');
...
fs.writeFileSync(path.join(subCodex, '.shizen-active'), 'full');
result = run('shizen-subagent.js', { HOME: subHome, USERPROFILE: subHome, PLUGIN_DATA: subCodex });
assert.equal(result.status, 0, result.stderr);
output = JSON.parse(result.stdout);
assert.equal(output.systemMessage, 'SHIZEN:FULL');
assert.equal(output.additionalContext, undefined, 'Codex must not emit additionalContext at top level (#573)');
assert.equal(output.hookSpecificOutput.hookEventName, 'SubagentStart');
assert.match(output.hookSpecificOutput.additionalContext, /SHIZEN MODE ACTIVE — level: full/);

// SubagentStart scoping (issue #506): SHIZEN_SUBAGENT_MATCHER limits the
...
const scopeFlag = path.join(scopeHome, '.claude', '.shizen-active');
...
result = run(
  'shizen-subagent.js',
  { ...scopeEnv, SHIZEN_SUBAGENT_MATCHER: 'general|plan' },
...
result = run(
  'shizen-subagent.js',
  { ...scopeEnv, SHIZEN_SUBAGENT_MATCHER: '^general$' },
  JSON.stringify({ agent_type: 'general-purpose' }),
);
assert.equal(result.status, 0, result.stderr);
...
result = run(
  'shizen-subagent.js',
  { ...scopeEnv, SHIZEN_SUBAGENT_MATCHER: '(' },
...
result = run('shizen-subagent.js', scopeEnv, '');
assert.equal(result.status, 0, result.stderr);
output = JSON.parse(result.stdout);
assert.match(output.hookSpecificOutput.additionalContext, /SHIZEN MODE ACTIVE — level: full/);
...
const qoderState = path.join(qoderHome, '.qoder', '.shizen-active');
...
// /shizen ultra: mode tracker updates flag and injects ultra ruleset.
result = run(
  'shizen-mode-tracker.js',
  qoderEnv,
  JSON.stringify({ prompt: '/shizen ultra' }),
...
assert.match(
  output.hookSpecificOutput.additionalContext,
  /SHIZEN MODE CHANGED — level: ultra/,
);

// "stop shizen": deactivates, clears flag, no ruleset output.
result = run(
  'shizen-mode-tracker.js',
  qoderEnv,
  JSON.stringify({ prompt: 'stop shizen' }),
);
assert.equal(result.status, 0, result.stderr);
assert.equal(fs.existsSync(qoderState), false, 'flag must be cleared after stop shizen');
output = JSON.parse(result.stdout);
assert.equal(output.hookSpecificOutput.additionalContext, 'SHIZEN MODE OFF');

// Subagent injection via PreToolUse (task|Task matcher): when shizen is
// active, the subagent hook injects the ruleset. Qoder shares the same
// shizen-subagent.js script; the isQoder branch outputs hookSpecificOutput
// JSON instead of raw stdout.
fs.writeFileSync(qoderState, 'full');
result = run('shizen-subagent.js', qoderEnv);
assert.equal(result.status, 0, result.stderr);
output = JSON.parse(result.stdout);
assert.equal(output.hookSpecificOutput.hookEventName, 'SubagentStart');
assert.match(
...
const mergeConfigDir = path.join(mergeHome, '.config', 'shizen');
...
// #329: `/shizen default <mode>` persists the default to config (survives
...
const defConfig = path.join(defHome, '.config', 'shizen', 'config.json');
const defFlag = path.join(defHome, '.claude', '.shizen-active');

result = run('shizen-mode-tracker.js', defEnv, JSON.stringify({ prompt: '/shizen default lite' }));
assert.equal(result.status, 0, result.stderr);
assert.equal(JSON.parse(fs.readFileSync(defConfig, 'utf8')).defaultMode, 'lite', '/shizen default must persist the default');
assert.equal(fs.existsSync(defFlag), false, '/shizen default must not change the session mode');

// A plain switch is transient: sets the session flag, leaves the default alone.
result = run('shizen-mode-tracker.js', defEnv, JSON.stringify({ prompt: '/shizen ultra' }));
assert.equal(result.status, 0, result.stderr);
...
result = run('shizen-mode-tracker.js', defEnv, JSON.stringify({ prompt: '/shizen default review' }));
assert.equal(result.status, 0, result.stderr);
...
// SHIZEN_DEFAULT_MODE=review falls back to the built-in default.
const revHome = path.join(temp, 'review-default-home');
const revConfigDir = path.join(revHome, '.config', 'shizen');
...
const prevEnvModeRev = process.env.SHIZEN_DEFAULT_MODE;
...
try {
  ...
  delete process.env.SHIZEN_DEFAULT_MODE;
  fs.rmSync(revConfigPath);
  process.env.SHIZEN_DEFAULT_MODE = 'review';
  assert.equal(getDefaultMode(), DEFAULT_MODE, 'SHIZEN_DEFAULT_MODE=review must fall back to the built-in default');
} finally {
  if (prevXdgRev === undefined) delete process.env.XDG_CONFIG_HOME; else process.env.XDG_CONFIG_HOME = prevXdgRev;
  if (prevEnvModeRev === undefined) delete process.env.SHIZEN_DEFAULT_MODE; else process.env.SHIZEN_DEFAULT_MODE = prevEnvModeRev;
}
