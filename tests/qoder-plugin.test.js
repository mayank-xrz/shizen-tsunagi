...
test('qoder plugin manifest exists and has required fields', () => {
  ...
  assert.equal(manifest.name, 'shizen');
...
test('qoder hooks config exists and registers UserPromptSubmit', () => {
  ...
  assert.ok(cmd.includes('shizen-mode-tracker.js'), 'must point at shizen-mode-tracker.js');
});

test('qoder rules file exists and is non-empty', () => {
  const rulesPath = path.join(root, '.qoder', 'rules', 'shizen.md');
  assert.ok(fs.existsSync(rulesPath), '.qoder/rules/shizen.md must exist');
  const content = fs.readFileSync(rulesPath, 'utf8').trim();
  assert.ok(content.length > 0, '.qoder/rules/shizen.md must not be empty');
  assert.ok(content.includes('nature builds'), 'rules must contain the shizen identity');
});
...
test('qoder rules match AGENTS.md canonical body', () => {
  ...
  const qoderCopy = fs.readFileSync(path.join(root, '.qoder', 'rules', 'shizen.md'), 'utf8')
    .replace(/\r\n/g, '\n').trim();
  assert.equal(qoderCopy, canonical, '.qoder/rules/shizen.md drifted from AGENTS.md');
});

test('qoder runtime detects QODER_SESSION_ID and writes hookSpecificOutput JSON', () => {
  const { isQoder } = require('../hooks/shizen-runtime');
