...
// always-on context, commands/*.toml for /shizen + /shizen-review, and
...
// carries the load-bearing rules — i.e. if the adapter stops wiring shizen.
...
const REUSED_COMMANDS = ['commands/shizen.toml', 'commands/shizen-review.toml'];
const REUSED_SKILLS = ['skills/shizen/SKILL.md'];
// Gemini CLI auto-loads this exact path for extension hooks. Shizen's
...
test('manifest names the shizen extension with a pinned version', () => {
...
test('contextFileName resolves to a file carrying the shizen rules', () => {
  const manifest = loadManifest();
