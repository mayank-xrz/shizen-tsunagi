...
const commands = ['shizen', 'shizen-review', 'shizen-audit', 'shizen-debt', 'shizen-help'];
const skillCommands = commands.filter((name) => name !== 'shizen');

const root = path.join(__dirname, '..');

// shizen: probe once; on Windows `python3` is the Store-alias stub that fails
...
test('Hermes plugin manifest matches runtime skills, hooks, commands, and package version', () => {
  ...
  assert.match(manifest, /^name:\s*shizen$/m);
...
test('Hermes plugin registers every shipped skill under the shizen namespace', () => {
  const output = python(String.raw`
import importlib.util, json, pathlib
spec = importlib.util.spec_from_file_location('shizen_hermes_plugin', '__init__.py')
...
`);
  ...
  assert.ok(data.commands.includes('shizen'));
  assert.ok(data.commands.includes('shizen-review'));
});

test('Hermes plugin builds mode-aware injected context from the canonical skill', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'shizen-config-'));
  const output = python(String.raw`
...
`, { XDG_CONFIG_HOME: tmp });
  ...
  assert.match(ctx, /SHIZEN MODE ACTIVE — level: ultra/);
  assert.match(ctx, /The best\s+code is the code nature would never have\s+grown/);
...
test('Hermes mode config respects env, config file, off, and invalid command behavior', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'shizen-config-'));
  fs.mkdirSync(path.join(tmp, 'shizen'), { recursive: true });
  fs.writeFileSync(path.join(tmp, 'shizen', 'config.json'), JSON.stringify({ defaultMode: 'lite' }));
  const output = python(String.raw`
import importlib.util, json
spec = importlib.util.spec_from_file_location('shizen_hermes_plugin', '__init__.py')
mod = importlib.util.module_from_spec(spec)
...
ctx = Ctx()
mod.register(ctx)
status_before = ctx.commands['shizen']('')
invalid = ctx.commands['shizen']('maximum')
status_after = ctx.commands['shizen']('')
print(json.dumps({
...
`, { XDG_CONFIG_HOME: tmp, SHIZEN_DEFAULT_MODE: 'ultra' });
  const data = JSON.parse(output);
  assert.match(data.default, /level: ultra/);
  assert.equal(data.off, '');
  assert.match(data.status_before, /Shizen mode: ultra/);
  assert.match(data.invalid, /Usage:/);
  assert.match(data.status_after, /Shizen mode: ultra/);
});
...
`);
  const { ctx } = JSON.parse(output);
  assert.match(ctx, /SHIZEN MODE ACTIVE — level: review/);
  assert.match(ctx, /Review diffs for unnecessary complexity/);
  assert.match(ctx, /net: -<N> lines possible/);
  assert.doesNotMatch(ctx, /^---/);
});

test('Hermes /shizen command changes mode and pre_llm_call injects current context', () => {
  const output = python(String.raw`
import importlib.util, json
spec = importlib.util.spec_from_file_location('shizen_hermes_plugin', '__init__.py')
mod = importlib.util.module_from_spec(spec)
...
ctx = Ctx()
mod.register(ctx)
message = ctx.commands['shizen']('ultra')
...
`);
  ...
  assert.match(data.context, /SHIZEN MODE ACTIVE — level: ultra/);
});
...
cases = {}
for text in ['/shizen-review x', '/shizen_audit repo', '/shizen-debt', '/shizen-help', '/status', 'hello']:
    cases[text] = mod.rewrite_gateway_command(event=Event(text))
print(json.dumps(cases, sort_keys=True))
`);
  const data = JSON.parse(output);
  assert.match(data['/shizen-review x'].text, /shizen-review/);
  assert.match(data['/shizen_audit repo'].text, /shizen-audit/);
  assert.match(data['/shizen_audit repo'].text, /repo/);
  assert.match(data['/shizen-debt'].text, /shizen-debt/);
  assert.match(data['/shizen-help'].text, /shizen-help/);
