...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'shizen-opencode-'));
process.env.XDG_CONFIG_HOME = tmp;
delete process.env.SHIZEN_DEFAULT_MODE;
const statePath = path.join(tmp, 'opencode', '.shizen-active');

let loadPlugin, parseCommandFile;
test.before(async () => {
  const url = pathToFileURL(path.join(__dirname, '..', '.opencode', 'plugins', 'shizen.mjs'));
  ...
  parseCommandFile = require(path.join(__dirname, '..', '.opencode', 'plugins', 'shizen-frontmatter.cjs')).parseCommandFile;
});
...
test('system.transform injects the ruleset at the default mode (full)', async () => {
  ...
  assert.match(system[0], /SHIZEN MODE ACTIVE — level: full/);
  assert.match(system[0], /nature builds/);
});

test('command.execute.before persists /shizen ultra, transform follows it', async () => {
  const hooks = await loadPlugin({});
  await hooks['command.execute.before']({ command: 'shizen', arguments: 'ultra', sessionID: 's' });
  assert.equal(fs.readFileSync(statePath, 'utf8'), 'ultra');
  const system = await transform(hooks);
  assert.match(system[0], /SHIZEN MODE ACTIVE — level: ultra/);
});

test('/shizen off persists off and transform injects nothing', async () => {
  const hooks = await loadPlugin({});
  await hooks['command.execute.before']({ command: 'shizen', arguments: 'off', sessionID: 's' });
...
test('system.transform merges into existing system entry (Qwen compat, #296)', async () => {
  ...
  assert.match(output.system[0], /SHIZEN MODE ACTIVE/);
});

test('unsupported /shizen arguments do not reset the current mode', async () => {
  const hooks = await loadPlugin({});
  fs.writeFileSync(statePath, 'ultra');
  await hooks['command.execute.before']({ command: 'shizen', arguments: 'status', sessionID: 's' });
  assert.equal(fs.readFileSync(statePath, 'utf8'), 'ultra');
});
