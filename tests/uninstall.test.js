...
const temp = fs.mkdtempSync(path.join(os.tmpdir(), 'shizen-uninstall-'));
...
const flagPath = path.join(claudeDir, '.shizen-active');
fs.writeFileSync(flagPath, 'full');

const configDir = path.join(temp, 'config-home', 'shizen');
...
fs.writeFileSync(settingsPath, JSON.stringify({
  statusLine: { type: 'command', command: 'bash /some/path/shizen-statusline.sh' },
}));
...
assert.equal(
  ...
  undefined,
  'shizen statusLine entry must be removed',
);
...
// #374: a combined statusline (another plugin && shizen) must keep the other
// plugin's part — uninstall must not nuke the whole command or leave a husk.
fs.writeFileSync(settingsPath, JSON.stringify({
  statusLine: { type: 'command', command: 'bash ~/caveman-statusline.sh && bash /p/shizen-statusline.sh' },
}));
...
assert.equal(
  ...
  'a combined statusLine must keep the non-shizen command',
);
...
const malformedSettings = '{ "statusLine": { "command": "shizen-statusline.sh", broken';
