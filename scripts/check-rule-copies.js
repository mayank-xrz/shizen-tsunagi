...
const copies = [
  ['.cursor/rules/shizen.mdc', stripFrontmatter],
  ['.windsurf/rules/shizen.md', text => text.trim()],
  ['.clinerules/shizen.md', text => text.trim()],
  ['.agents/rules/shizen.md', text => text.trim()],
  ['.qoder/rules/shizen.md', text => text.trim()],
  ['.github/copilot-instructions.md', text => text.trim()],
  ['.kiro/steering/shizen.md', stripFrontmatter],
];
...
// so it cannot be byte-compared. shizen: canary, not full equality. Assert the
...
const INVARIANTS = [
  ...
  // the four "never casual about" safety carve-outs: pin each so a reword in either
  ...
  'security',
  'accessibility',
  'Spare code without its check is unfinished', // one-check promoted to headline
];

const skill = read('skills/shizen/SKILL.md');
const sources = [['skills/shizen/SKILL.md', skill], ['AGENTS.md', agents]];
