...
`app/not-found.tsx`: a hitofude line that stops mid-stroke — visibly unfinished, the one drawing on the site that never completes; its subject is fixed at the contact-sheet gate (§4). It renders static: the stop is the drawing's, not the animation's — a page with no scroll depth would otherwise freeze the draw at an arbitrary fraction. Line: "This page isn't connected.", a plain link home. Voice per §7.
