...
- **Detail page:** a hero band washed in the product's hue (`--hue-wash`), carrying the gold Coming soon eyebrow, the name at 2.25rem/700 in the deep hue, the tagline at subhead size in `--ink`, and a color-provenance caption in label `--ink` naming the hue — 焦茶 kogecha — burnt-tea brown · 金色 kin-iro — antique gold · 小豆 azuki — red-bean rose · 柿 kaki — persimmon · 苔 koke — moss green — kanji in `lang='ja'`. The pack shot (multiply + blur-up; it stays per prd §5) sits on the band's right at ≥768px, overlapping the band's lower edge into the cream below; below 768px the band stacks text-over-image. Then, on cream: the description in body, **benefits as the ruled list** — each on its own line, a `--line` rule between, no bullets — the ingredient interpunct line in label `--ink`, one thin seigaiha divider in the deep hue, and a solid **Notify me**. The reviews section follows (§10).
- **Footer:** teal background, cream text. Order: logo, brand line, then a **`<nav aria-label="Footer">`** carrying two plain lists — (1) the five product names linking to their detail pages, (2) **Home** (`/`) and **About us** (`/#story`) — then the contact line, then © year. Links are `--cream` (7.01 on teal), body size for tap targets, no boxes, no group headings — two short lists (five + two) don't warrant eyebrows, and restraint is the rule (§7); the lists sit side-by-side at ≥768px and stack below it, kept apart by spacing alone. The contact line reads **"Contact us: stsales@shizentsunagi.com"** with the address as the `mailto:` link — that is the Contact destination, not duplicated as a list item. Focus rings in the footer are `--cream`, not the global `--teal` (a teal ring vanishes on the teal ground) — scoped override, logged. The header keeps its "no nav links" — the clean header is deliberate, the footer is the sitemap. **About us / story anchor:** the home page's brand-story section carries `id="story"`, so `/#story` resolves from any detail page too; the header is not sticky, so the anchor lands clean (add `scroll-margin-top` only if a check shows it clipped).
