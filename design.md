...
- **Japanese glyphs:** a second `next/font` instance of Shippori Mincho — `next/font/local` over a committed woff2 pre-subsetted by Google's `text=自然つなぎ繋` API, because `next/font` itself has no `text` option (owner-approved 2026-07-16) — loads only the six glyphs the site uses; its className is applied to every `lang='ja'` span. Still one display family + one silent family — no third font.
...
Implementation: the inline SVG per page carries the thread path and small static gold nodes (r≈3, `--gold`) where the thread joins each section; the terminal group where the thread resolves into a simplified mizuhiki awaji knot at the brand-story section is its own small SVG pinned there at the thread's end — the stretched thread viewport cannot keep a knot round at every width (owner-approved 2026-07-16). The joins are gold, kintsugi logic. Absolutely positioned, `pointer-events: none`, drawn-in on scroll via `stroke-dashoffset` transition. Animation unchanged: only the line draws in; nodes and knot are static; under `prefers-reduced-motion: reduce` everything renders fully drawn. Below 768px the line also renders fully drawn — Chromium cannot paint a dashed non-scaling stroke at phone-level viewport stretch, so mobile takes the same static fallback (bug workaround, logged). The product-page divider gains one gold end-dot. This is the only animation on the site — no fades, no parallax, no hover theatrics beyond the button states below.
