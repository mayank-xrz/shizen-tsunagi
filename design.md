...
2. **Full-opacity numeral** (z1) — 一…五 in fat gothic, the dominant hue, `min(56vw, 30rem)`, top/side anchor that content overlaps and wraps around; `aria-hidden`; drifts/rotates slowly on `view()` (§12). Cropped by its own overflow layer so it never forces horizontal scroll (the v5 ghost-numeral technique). Dominant-hue-on-own-wash passes AA-large (§8). **On narrow screens (≤767px) it re-anchors to the top, behind the pack shot** — a vertically-centred numeral over the tall stacked stall otherwise lands in the text lane as a stray block (2026-07-20 polish); desktop keeps the full-opacity centred anchor.
...
6. **Product name** — the dominant hue, gothic, `clamp(2.4rem, 9.5vw, 5.25rem)`, solid fill (the optional outline stroke is dropped — it muddied the light hues; the solid 900-weight hue reads louder and cleaner, 2026-07-20 polish).
7. **Coming-soon burst** — a chirashi **star/burst price-tag** (SVG polygon) in the dominant hue with cream burst text ≥1.17rem bold (AA-large, §8), gold ring; rotated 2–8° (§6 furniture range), "stamps in" on entry (§12). Replaces v5's quiet gold eyebrow. **It stamps the pack shot's outer corner, never the text column** — the price-tag belongs on the goods, and this keeps the name + catalog line unobstructed at every width (2026-07-20 polish; the same move on the detail page).
...
- **Burst** (`components/Seal.tsx`, same file): the chirashi price-tag — an SVG star/burst polygon in the dominant hue, gold ring, cream text ≥1.17rem bold. The **"Coming soon"** announcement, stamped on the **pack shot's corner** (not the text column — §4.7).
...
Max content width 1080px, 24px side padding, single 768px breakpoint. Rhythm 112/64 as vertical padding; the gate and stalls additionally center content in `min-height: 100svh`. The **gate runs a lighter pad** (`clamp(28px, 6vw, 72px)`) than the stalls — it carries a full-height composition, and the lighter pad keeps the brand + value prop + primary CTA all in the first viewport at 1440/390/360 (brief priority; 2026-07-20 polish). Its two-column inner **must set `width: 100%`** (like the stalls' `.chapter-inner`): as a `margin-inline:auto` flex child it otherwise collapses to content width and starves both columns. Radius: 0 surfaces, 2px controls.
...
- **Form:** sits on a **festival board** — a bordered plate (dominant hue border + gold outline) over a faint hue-inked shippō layer, so the submission (and its loading/success/error states) belongs to the same system as the stalls (2026-07-20 polish); inputs stay solid-white plates so text keeps its ground (§8). A `fieldset`/`legend` radio group of five labelled star inputs (accessible names "1 star"…"5 stars"), each with the teal focus ring; name + comment fields per the input style (radius 2px, DotGothic16 labels); solid teal submit ("Submit review"). Client-side required checks mirror the server; the server stays the trust boundary.
