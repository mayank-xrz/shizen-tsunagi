#!/usr/bin/env node
// shizen — UserPromptSubmit hook to track which shizen mode is active
// Inspects user input for /shizen commands and writes mode to flag file

const { getDefaultMode, isDeactivationCommand, writeDefaultMode } = require('./shizen-config');
const { clearMode, isQoder, readMode, setMode, writeHookOutput } = require('./shizen-runtime');
const { getShizenInstructions } = require('./shizen-instructions');
...
function finish() {
  ...
  try {
    ...
    // Match /shizen commands
    let modeSwitched = false;
    let deactivated = false;
    if (/^[/@$]shizen/.test(prompt)) {
      ...
      if (cmd === '/shizen-review' || cmd === '/shizen:shizen-review') {
        mode = 'review';
      } else if (cmd === '/shizen' || cmd === '/shizen:shizen') {
        // `/shizen default <mode>` persists the default to config (survives
        ...
        if (arg === 'default') {
          ...
          if (dmode === 'off' || dmode === 'lite' || dmode === 'full' || dmode === 'ultra') {
            ...
            writeHookOutput('UserPromptSubmit', dmode, 'SHIZEN DEFAULT SET — new sessions start in ' + dmode + '.');
          }
      ...
      if (isReportOnly) {
        writeHookOutput(
          'UserPromptSubmit',
          mode,
          'SHIZEN MODE ACTIVE — level: ' + mode,
        );
      } else if (mode && mode !== 'off') {
        setMode(mode);
        modeSwitched = true;
        // shizen: Qoder needs the full ruleset every turn, so when a mode
        ...
        if (!isQoder) {
          writeHookOutput(
            'UserPromptSubmit',
            mode,
            'SHIZEN MODE CHANGED — level: ' + mode,
          );
        }
      } else if (mode === 'off') {
        clearMode();
        deactivated = true;
        writeHookOutput('UserPromptSubmit', 'off', 'SHIZEN MODE OFF');
      }
    }
    ...
    // SessionStart via shizen-activate.js; Qoder can't, so we do it here.
    // Skip when deactivated — user just turned shizen off.
    if (isQoder && !deactivated) {
      ...
      if (currentMode && currentMode !== 'off') {
        // shizen: one JSON per invocation — mode-switch confirmation is
        // folded into the ruleset header so Qoder gets both in one write.
        const header = modeSwitched
          ? 'SHIZEN MODE CHANGED — level: ' + currentMode + '\n\n'
          : '';
        writeHookOutput('UserPromptSubmit', currentMode, header + getShizenInstructions(currentMode));
      }
    }
  } catch (e) {
    // Silent fail
