...
flag="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/.shizen-active"
[ -f "$flag" ] || exit 0
...
if [ -z "$mode" ] || [ "$mode" = "full" ]; then
    printf '\033[38;5;%sm[SHIZEN]\033[0m' "$color"
else
    printf '\033[38;5;%sm[SHIZEN:%s]\033[0m' "$color" "$(printf '%s' "$mode" | tr '[:lower:]' '[:upper:]')"
fi
