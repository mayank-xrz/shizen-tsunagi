#!/usr/bin/env node
// shizen — Claude Code SubagentStart hook
//
// SessionStart context is parent-thread only and never reaches subagents, so
// without this every Task-spawned agent runs shizen-unaware (issue #252).
// When shizen mode is active, inject the same ruleset into each subagent.
//
// Scoping (opt-in, issue #506): set SHIZEN_SUBAGENT_MATCHER to a regex and
...
const { readMode, writeHookOutput } = require('./shizen-runtime');

const mode = readMode();

// Absent flag or off → shizen isn't active; inject nothing.
if (!mode || mode === 'off') {
  process.exit(0);
}
...
function inject() {
  try {
    writeHookOutput('SubagentStart', mode, getShizenInstructions(mode));
  } catch (e) {
...
let matcherRe = null;
try {
  if (process.env.SHIZEN_SUBAGENT_MATCHER) {
    matcherRe = new RegExp(process.env.SHIZEN_SUBAGENT_MATCHER, 'i');
  }
} catch (e) {
  matcherRe = null;
}
