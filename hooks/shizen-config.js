#!/usr/bin/env node
// shizen — shared configuration resolver
//
// Resolution order for default mode:
//   1. SHIZEN_DEFAULT_MODE environment variable
//   2. Config file defaultMode field:
//      - $XDG_CONFIG_HOME/shizen/config.json (any platform, if set)
//      - ~/.config/shizen/config.json (macOS / Linux fallback)
//      - %APPDATA%\shizen\config.json (Windows fallback)
//   3. 'full'
...
// "stop shizen" / "normal mode" turn shizen off, but only as a standalone
...
function isDeactivationCommand(text) {
  ...
  return t === 'stop shizen' || t === 'normal mode';
}

// shizen: only embed the plugin install path in a statusline shell command when
...
function getConfigDir() {
  if (process.env.XDG_CONFIG_HOME) {
    return path.join(process.env.XDG_CONFIG_HOME, 'shizen');
  }
  ...
  return path.join(os.homedir(), '.config', 'shizen');
}
...
function getClaudeDir() {
  // shizen: CLAUDE_CONFIG_DIR overrides ~/.claude, matching Claude Code.
  return process.env.CLAUDE_CONFIG_DIR || path.join(os.homedir(), '.claude');
}

function getDefaultMode() {
  // 1. Environment variable (highest priority)
  const envMode = process.env.SHIZEN_DEFAULT_MODE;
  // shizen: a default must be a runtime level (off/lite/full/ultra); review is
...
// Silence the pi "Shizen loaded" startup toast while keeping shizen active.
// SHIZEN_QUIET_STARTUP=1 (or any truthy value; 0/false/empty mean "show it")
// takes precedence, else config.quietStartup === true. Mirrors getHideStatus.
function getQuietStartup() {
  const env = process.env.SHIZEN_QUIET_STARTUP;
  if (env !== undefined) {
...
// Hide the status-bar indicator while keeping shizen active (#324).
// SHIZEN_HIDE_STATUS=1 (or any truthy value; 0/false/empty mean "don't hide")
// takes precedence, else config.hideStatus === true.
function getHideStatus() {
  const env = process.env.SHIZEN_HIDE_STATUS;
  if (env !== undefined) {
    const v = env.trim().toLowerCase();
    return v !== '' && v !== '0' && v !== 'false' && v !== 'no';
  }
...
function writeDefaultMode(mode) {
  // shizen: only a runtime level can be a default; review is session-only (#377).
