...
SKILL_COMMANDS = {
    "shizen-review": "Review the current diff or provided target for over-engineering.",
    "shizen-audit": "Audit the repo for over-engineering and deletion opportunities.",
    "shizen-debt": "List every deliberate `shizen:` shortcut and its upgrade path.",
    "shizen-help": "Show the Shizen command reference.",
}

ROOT = Path(__file__).resolve().parent
SKILLS_DIR = ROOT / "skills"
SHIZEN_SKILL = SKILLS_DIR / "shizen" / "SKILL.md"
REVIEW_SKILL = SKILLS_DIR / "shizen-review" / "SKILL.md"
...
def _config_dir() -> Path:
    if os.environ.get("XDG_CONFIG_HOME"):
        return Path(os.environ["XDG_CONFIG_HOME"]) / "shizen"
    if os.name == "nt":
        return Path(os.environ.get("APPDATA", Path.home() / "AppData" / "Roaming")) / "shizen"
    return Path.home() / ".config" / "shizen"


def _default_mode() -> str:
    env_mode = _normalize_config_mode(os.environ.get("SHIZEN_DEFAULT_MODE"))
    if env_mode:
        return env_mode
    try:
...
def _fallback_instructions(mode: str) -> str:
    return (
        f"SHIZEN MODE ACTIVE — level: {mode}\n\n"
        "You code the way nature builds: the shortest path that actually holds, "
        "nothing grown that is not needed. Shizen means unforced, not careless. "
        "The best code is the code nature would never have grown in the first place.\n\n"
...
def build_injected_context(mode: str | None = None) -> str:
    """Return the mode-filtered Shizen context injected before LLM turns."""
    ...
    if configured == "review":
        try:
            ...
            return f"SHIZEN MODE ACTIVE — level: review\n\n{_strip_frontmatter(body)}"
        except OSError:
            return "SHIZEN MODE ACTIVE — level: review. Review diffs for unnecessary complexity."

    effective = _normalize_runtime_mode(configured) or DEFAULT_MODE
    try:
        body = SHIZEN_SKILL.read_text(encoding="utf-8")
        return f"SHIZEN MODE ACTIVE — level: {effective}\n\n{_filter_skill_body_for_mode(body, effective)}"
    except OSError:
...
def _skill_prompt(command: str, args: str = "") -> str:
    ...
    return (
        f"Load and follow the Hermes plugin skill `shizen:{command}`. "
...
def rewrite_gateway_command(event: Any = None, gateway: Any = None, **_: Any) -> dict[str, str] | None:
    """Rewrite authorized gateway /shizen-* commands into normal agent prompts."""
...
def _handle_mode_command(raw_args: str) -> str:
    ...
    if not arg:
        mode = _current_mode or _default_mode()
        return f"Shizen mode: {mode}. Use `/shizen lite|full|ultra|off`."
    mode = _normalize_runtime_mode(arg)
    if not mode:
        return "Usage: /shizen [lite|full|ultra|off]"
    _current_mode = mode
    return f"Shizen mode set to {mode}."
...
def register(ctx: Any) -> None:
    """Register Shizen hooks, skills, and slash commands with Hermes."""
    ...
    ctx.register_command(
        ...
        description="Set Shizen mode: lite, full, ultra, or off.",
